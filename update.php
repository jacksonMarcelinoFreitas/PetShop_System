<?php
    ...
    function updateServico($id){
        ...
        try{
            ...
            $stmt = $connection->prepare($sql);
            $stmt->bind_param('sdi', $nomeServico, $valorServico, $id);
            $stmt->execute();
    ...
    function updateCompra($id){
        ...
        try{
            ...
            $id = trim($connection->real_escape_string($id));
            $dataCompra = trim($connection->real_escape_string($nomeServico));
            $valor= trim($connection->real_escape_string($valorServico));

            $sql = "UPDATE servico SET dataCompra = ?, valorTotal = ? WHERE idCompra = ?";

            $stmt = $connection->prepare($sql);
            $stmt->bind_param('sdi', $dataCompra, $valor, $id);
            $stmt->execute();
